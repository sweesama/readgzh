  ...
  <Html lang="zh" dir="ltr">
    ...
    <Body style={main}>
      <Container style={container}>
        ...
        <Text style={text}>你请求将 {siteName} 的邮箱从 <Link href={`mailto:${oldEmail || email}`} style={link}>{oldEmail || email}</Link> 更改为 <Link href={`mailto:${newEmail}`} style={link}>{newEmail}</Link>。</Text>
        <Text style={text}>点击下方按钮确认此变更：</Text>
        <Button style={button} href={confirmationUrl}>确认变更</Button>
        <Text style={footer}>如果你没有请求此变更，请立即检查你的账户安全。</Text>
      </Container>
    </Body>
  </Html>
)
