...
-- Update validate_api_key to support monthly credit budgets for paid tiers
...
BEGIN
  ...
  -- Record today's usage (always per-day granularity for tracking)
  ...
  -- Total limit = daily/monthly limit + bonus credits
  v_total_limit := v_key.daily_limit + COALESCE(v_key.bonus_credits, 0);
...
-- Update get_user_balance to support monthly credit model
...
DECLARE
  ...
  v_bonus INTEGER;
BEGIN
  -- Get tier, daily limit and bonus credits
  SELECT COALESCE(MAX(tier), 'free'), COALESCE(MAX(daily_limit), 30), COALESCE(SUM(bonus_credits), 0)
  INTO v_tier, v_daily_limit, v_bonus
  FROM public.api_keys
  WHERE user_id = p_user_id AND is_active = true;

  v_is_paid := v_tier IN ('lite', 'pro', 'pro_lifetime');

  -- For paid users, credits are auto-granted monthly (no manual claiming needed)
  IF v_is_paid THEN
    v_claimed := v_daily_limit;
    v_has_claimed := true;
    -- Sum usage for the current month
...
-- Update daily_credits default to 30
ALTER TABLE public.daily_credits ALTER COLUMN credits_claimed SET DEFAULT 30;
