...
await build({
  ...
  build: { ssr: "scripts/prerender.tsx", outDir: "dist-ssr", rollupOptions: { output: { entryFileNames: "prerender.mjs" } } },
});
...
for (const page of renderPages()) {
  ...
  document.querySelectorAll('title, meta[name="description"], meta[property^="og:"], meta[name^="twitter:"], link[rel="canonical"], script[type="application/ld+json"], noscript').forEach(node => node.remove());
